...
//  byte lowFHeading = psimap + betaimage;  
  ...
  //this is just for straight road, might work for turning if image processing is strong;
  heading_est_degrees = (betaimage + tau * highFDotHeading - heading_est_degrees) / tau * (.001 * timeRead); //(lowFHeading + tau * highFDotHeading - heading_est_degrees) / tau * (.001 * timeRead);
...
//  distance_est = (lowFDistance + tau * highFDotDistance - distance_est) / tau * (.001 * timeRead);
  ...
  //control time of loop, delay 20 ms
...
//  float beta_desired = offset_Beta * k_offset; //positive if going CCW
  float delta = servoAngleDeg + k_phi* heading_est_degrees; //might be minus
  steeringServo.write(constrain(delta,60,120));

  //offset determine speed
  if (offset_est < 0.3 || heading_est_degrees> 30 || heading_est_degrees < -30){
    motor_pwm = 100;
  }
  else if(offset_est > 0.3 && heading_est_degrees <10 && heading_est_degrees > -10){
    motor_pwm = 250;
  }
